...
async def seed(session: AsyncSession) -> str:
    ...
    passengers = pick_passengers(PAX_COUNT, start_index=150)  # start at 151 to avoid ID overlap

    for pid, name, bref in passengers:
        has_connection = random.random() < 0.8
        ...
        if has_connection:
            conn = random.choice(FRA_CONNECTIONS)
...
def _add_options(
    ...
    session.add(OptionRow(
        ...
        description="ICE from Nürnberg Hbf to Frankfurt Flughafen Fernbahnhof. Shuttle from NUE to Nürnberg Hbf included.",
        details_json={
            "mode": "train",
            "route": "NUE → Nürnberg Hbf → ICE → Frankfurt Flughafen Fernbf",
    ...
    session.add(OptionRow(
        ...
        passenger_id=passenger_id,
        type="hotel",
        summary="Hotel Nürnberg + LH191 tomorrow",
        description="Overnight stay at Mövenpick Hotel Nürnberg Airport. Flight LH191 NUE→FRA at 07:00 tomorrow.",
        details_json={
            "hotel_name": "Mövenpick Hotel Nürnberg Airport",
            "address": "Flughafenstr. 100, 90411 Nürnberg",
