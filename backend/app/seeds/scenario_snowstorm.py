...
Each cancelled flight gets its own disruption record so the dashboard can
match passengers to their specific disrupted flight.
...
PAX_PER_FLIGHT = 25
# One disruption per cancelled flight
DISRUPTION_IDS = [
    f"dis-snowstorm-{flight_num.lower()}" for flight_num, *_ in CANCELLED_FLIGHTS
]
# Legacy aggregate ID from v1 (single disruption for all flights) — included
# so that reset can clean up old data after the schema change.
LEGACY_IDS = ["dis-snowstorm-001"]
...
SNOWSTORM_REASON = "Heavy snowstorm in Munich -- all runways closed"

# Per-flight fallback explanations
_FLIGHT_EXPLANATIONS = {
    "CDG": (
        "Flight {fn} from Munich to Paris Charles de Gaulle has been cancelled "
        "due to a severe snowstorm closing all runways at Munich Airport. "
        "Estimated reopening in 6-8 hours."
    ),
    "FRA": (
        "Flight {fn} from Munich to Frankfurt has been cancelled "
        "due to a severe snowstorm closing all runways at Munich Airport. "
        "Passengers with onward connections through Frankfurt are affected."
    ),
    "FCO": (
        "Flight {fn} from Munich to Rome Fiumicino has been cancelled "
        "due to a severe snowstorm closing all runways at Munich Airport."
    ),
    "BCN": (
        "Flight {fn} from Munich to Barcelona has been cancelled "
        "due to a severe snowstorm closing all runways at Munich Airport."
    ),
    "ZRH": (
        "Flight {fn} from Munich to Zurich has been cancelled "
        "due to a severe snowstorm closing all runways at Munich Airport."
    ),
}
...
async def seed(session: AsyncSession) -> list[str]:
    """Seed passengers, segments, and per-flight disruption records.

    Returns list of disruption IDs (one per cancelled flight).
    ...
    for flight_idx, (flight_num, origin, dest, flight_dur) in enumerate(CANCELLED_FLIGHTS):
        dis_id = DISRUPTION_IDS[flight_idx]
        departure = base + timedelta(hours=1 + flight_idx * 0.5)

        explanation = _FLIGHT_EXPLANATIONS.get(dest, "Flight {fn} cancelled due to snowstorm at Munich Airport.").format(fn=flight_num)
        ...
        disruption = DisruptionRow(
            ...
            origin=origin,
            destination=dest,
            reason=SNOWSTORM_REASON,
            explanation=explanation,
            detected_at=base,
        )
        session.add(disruption)
        created_ids.append(dis_id)
        ...
        for pid, name, bref, loyalty_tier, booking_class in passengers:
            ...
            # Link passenger to this flight's disruption
            session.add(DisruptionPassengerRow(
                disruption_id=dis_id, passenger_id=pid,
            ))
